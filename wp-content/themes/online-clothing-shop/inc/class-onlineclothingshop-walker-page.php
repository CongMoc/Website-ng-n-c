...
if ( ! class_exists( 'OnlineClothingShop_Walker_Page' ) ) {
	/**
	 * CUSTOM PAGE WALKER
	 ...
	 * @since Online Clothing Shop 1.0
	 */
	class OnlineClothingShop_Walker_Page extends Walker_Page {

		/**
		 * Outputs the beginning of the current element in the tree.
		 *
		 * @since Online Clothing Shop 1.0
		 * @since Online Clothing Shop 1.9 Renamed `$page` to `$data_object` and `$current_page` to `$current_object_id`
		...
		public function start_el( &$output, $data_object, $depth = 0, $args = array(), $current_object_id = 0 ) {
			...
			if ( '' === $page->post_title ) {
				...
				$page->post_title = sprintf( __( '#%d (no title)', 'online-clothing-shop' ), $page->ID );
			}
